...
function Rename-Many {
  ...
  * APPENDAGE EXAMPLES
  ...
  (
    ...
    [Parameter()]
    [PSCustomObject]$Context = $Bulk.Defaults.Remy.Context,
  ...
  begin {
    ...
    [scriptblock]$doRenameFsItems = {
      ...
      $postResult = invoke-PostProcessing -InputSource $newItemName -Rules $Bulk.Rules.Remy `
        -Signals $signals;
  ...
  process {
    ...
    [boolean]$locked = Get-IsLocked -Variable $(
      [string]::IsNullOrEmpty($Context.Locked) ? 'BULK_REMY_LOCKED' : $Context.Locked
    );
