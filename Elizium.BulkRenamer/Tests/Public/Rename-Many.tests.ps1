...
Describe 'Rename-Many' -Tag 'remy' {
  BeforeAll {
    ...
    Mock -ModuleName Elizium.BulkRenamer rename-FsItem {
      param(
        [FileSystemInfo]$From,
        [string]$To,
    ...
    Mock -ModuleName Elizium.BulkRenamer Get-IsLocked {
      return $true;
    }
...
Describe 'Rename-Many (Internal)' {
  ...
  Context 'and: Host does not support emojis' {
    It 'should: render with non unicode signals' -Skip -Tag 'REWRITE' {
      Mock -ModuleName Elizium.BulkRenamer Test-HostSupportsEmojis {
        return $false;
      }
      InModuleScope Elizium.BulkRenamer {
  ...
  context 'given: using DryRunner' -Tag 'DRY' {
    Context 'given: Valid Parameter Set' {
      It 'should: resolve <parameters> to <paramSet>' -TestCases @(
        ...
        InModuleScope -ModuleName Elizium.BulkRenamer -Parameters @{ Parameters = $parameters; ParamSet = $paramSet } {
          param(
            $Parameters, $ParamSet
          )
